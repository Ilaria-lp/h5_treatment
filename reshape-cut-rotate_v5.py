...
from h5_map_handling import check_bms, count_steps, get_data, get_name_and_date, orientation, read_positions, warning
...
def cut_reshape(in_file, out_fold):    
    ...
    key, sample_name, date_acq = get_name_and_date(f)
    ...
    try:                
        x, y = read_positions(f, str(key+PATH_SCALAR))
    except KeyError:
...
# in the correct orientation on PyMCA.     
    ...
    bms = get_data(f, str(key+PATH_SCALAR+I0_MONITOR))
    ...
    for vectorData in f[key+PATH_VECTOR].keys():
        v = f[key+PATH_VECTOR+"/"+vectorData][...]
        v = v[0:final_points]
    ...
    for scalarData in f[key+PATH_SCALAR].keys():
        if scalarData in AcceptedList or scalarData.endswith('LiveTime'):                
            s = f[key+PATH_SCALAR+"/"+scalarData][...]
            s = s[0:final_points]
            ...
            fout.create_dataset(key+"/Motor_positions/"+scalarData, data=s)

    # Reshaping Positioners (i.e. motor position before map collection)
    for motor_position in f[key+POSITIONERS].keys():
        p = f[key+POSITIONERS+"/"+motor_position][...]                   
        fout.create_dataset(key+"/Starting_positions/"+motor_position, data=p)
